...
class Frequency_Dialog_EMO(EMO):
    ...
    def activate(self):
        from ChimAARON.normal_modes import FreqFileLoader
...
class Library_Dialog_EMO(EMO):
    ...
    def activate(self):
        from ChimAARON.library import LibraryDialog
...
class AARON_Input_Dialog_EMO(EMO):
    ...
    def activate(self):
        from ChimAARON.aaron_input import InputGenerator_templateSelector
...
class StructureModification_EMO(EMO):
    ...
    def categoryDescriptions(self):
        return {'ChimAARON':self.name(), 'Structure Editing':self.name()}

    def icon(self):
        return self.path('Icons/mapLigand.png')

    def activate(self):
        from ChimAARON.structure_modification import StructureModificationDialog
...
class LibAdd_EMO(EMO):
    ...
    def activate(self):
        from ChimAARON.library import LibAddDialog
        dialog = LibAddDialog()
...
def cmd_Substitute(cmdName, args):
    ...
    from ChimAARON.structure_modification import doSubstitute
...
def cmd_CloseRing(cmdName, args):
    ...
    from ChimAARON.structure_modification import doCloseRing
    from chimera import openModels
...
def cmd_Follow(cmdName, args):
    ...
    from ChimAARON.normal_modes import doFollow
...
def cmd_FreqBild(cmdName, args):
    ...
    from ChimAARON.normal_modes import doFreqBild
...
def cmd_MapLigand(cmdName, args):
    ...
    from ChimAARON.structure_modification import doMapLigand
    from chimera import openModels
...
def cmd_ArnRecord(cmdName, arg_str):
    from ChimAARON.aaron_input import doArnRecord

    doArnRecord(cmdName, arg_str)

# libaddLigand equivalent
def cmd_LibAddLig(cmdName, arg_str):
    from ChimAARON.library import doLibaddLig

    doLibaddLig(cmdName, arg_str)# libaddLigand equivalent

# libaddSubstituent equivalent
def cmd_LibAddSub(cmdName, arg_str):
    from ChimAARON.library import doLibaddSub

    doLibaddSub(cmdName, arg_str)

# libaddRing equivalent
def cmd_LibAddRing(cmdName, arg_str):
    from ChimAARON.library import doLibaddRing

    doLibaddRing(cmdName, arg_str)
...
addCommand("libaddligand", cmd_LibAddLig)
addCommand("libaddsubstituent", cmd_LibAddSub)
addCommand("libaddring", cmd_LibAddRing)
